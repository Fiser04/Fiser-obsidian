...
// void setup() {
//   Serial.begin(115200);  // Inicializace sériové komunikace rychlostí 115200
//   baudů
...
//   pinMode(SW, INPUT_PULLUP);  // Nastavení pinu SW jako vstup s pull-up
//   rezistorem

//   lastStateCLK = digitalRead(CLK); // Uložení počátečního stavu hodinového
//   signálu
// }

// void loop() {
//   currentStateCLK = digitalRead(CLK);         // Čtení aktuálního stavu
//   hodinového signálu if (currentStateCLK != lastStateCLK && currentStateCLK
//   == 1){ // Detekce změny stavu a náběžné hrany
//     if (digitalRead(DT) != currentStateCLK) { // Kontrola směru otáčení
//     pomocí porovnání DT a CLK
//       counter --;                             // Dekrementace počítadla při
//       otáčení proti směru hodinových ručiček currentDir ="CCW"; // Nastavení
//       aktuálního směru na CCW (proti směru hodinových ručiček)
//     } else {
//       counter ++;                             // Inkrementace počítadla při
//       otáčení ve směru hodinových ručiček currentDir ="CW"; // Nastavení
//       aktuálního směru na CW (ve směru hodinových ručiček)
//     }
...
//   lastStateCLK = currentStateCLK;              // Aktualizace předchozího
//   stavu CLK pro příští iteraci

//   int btnState = digitalRead(SW);              // Čtení stavu tlačítka
//   enkodéru

//   if (btnState == LOW) {                       // Kontrola, zda je tlačítko
//   stisknuto (LOW při stisku díky INPUT_PULLUP)
//     if (millis() - lastButtonPress > 50) {     // Ošetření zákmitů tlačítka
//     (debouncing) - 50ms prodleva
//       Serial.println("Button pressed!");
//     }

//     lastButtonPress = millis();              // Aktualizace časové značky
...
void setup()
{
  ...
  pinMode(SENSOR_SUPPLY, OUTPUT);
  digitalWrite(SENSOR_SUPPLY, HIGH);
  Wire.begin(42, 2); // Set dedicated I2C pins 42 - SDA, 2 - SCL for
ESP32-S3-DEVKit
// Funkce pro aktualizaci zobrazení počítadla
void updateDisplay()
{
...
void loop()
{
  ...
  if (currentStateCLK != lastStateCLK && currentStateCLK == 1)
  {
    ...
    if (!buttonPressed) { // Aktualizace displeje jen pokud není zmáčknuto
tlačítko updateDisplay();
...
=======
>>>>>>> d8a1aab (hsdjfhks# Please enter the commit message for your changes. Lines starting)
...
void updateBall()
{
  ...
  // Kolize s pálkou
  bool hitPaddleY = (ballY + ballSize >= paddleY) && (ballY <= paddleY +
paddleH); bool hitPaddleX = (ballX + ballSize >= paddleX) && (ballX <= paddleX +
paddleW);
